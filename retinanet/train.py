    ...
    for epoch_num in range(parser.epochs):
        ...
        for iter_num, data in enumerate(dataloader_train):
            try:
                ...
                dtime = int(time.time() - starttime)
                print(
                    'Epoch: {} | Iteration: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f} | Running loss: {:1.5f} | Time: {}h {}m {}s '.format(
                        epoch_num, iter_num, float(classification_loss), float(regression_loss), np.mean(loss_hist), int(dtime/3600), int((dtime/60)%60), dtime%60))
        ...
        torch.save(retinanet.module, '{}_retinanet_epoch{}.pt'.format(parser.dataset, epoch_num))
