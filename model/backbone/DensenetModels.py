...
def _get_norm(norm_type, num_features, num_groups=32, eps=1e-5):
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate,
        ...
        self.add_module('norm1', _get_norm(norm_type, num_input_features)),
        self.add_module('relu1', nn.ReLU(inplace=True)),
        self.add_module('conv1', nn.Conv2d(num_input_features, bn_size *
                        growth_rate, kernel_size=1, stride=1, bias=False)),
        self.add_module('norm2', _get_norm(norm_type, bn_size * growth_rate)),
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features,
        ...
        self.add_module('norm', _get_norm(norm_type, num_input_features))
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 norm_type='Unknown', num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, in_cnl=3):  # noqa
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(self.in_cnl, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),  # noqa
            ('norm0', _get_norm(norm_type, num_init_features)),
        ...
        # Final batch norm
        self.features.add_module('norm5', _get_norm(norm_type, num_features))
...
def DenseNet121(cfg, **kwargs):
    ...
    img_cnl = 1 if "Grayscale" in cfg.dataset.datatransforms.train else 3
    model = DenseNet(num_init_features=cfg.model.num_init_features, growth_rate=32, block_config=(6, 12, 24, 16),  #noqa
                     norm_type=cfg.model.norm_type, in_cnl=img_cnl, **kwargs)
    if cfg.model.load_pretrained_backbone and "Grayscale" not in cfg.dataset.datatransforms.train:
...
def DenseNet169(cfg, **kwargs):
    ...
    """
    img_cnl = 1 if "Grayscale" in cfg.dataset.datatransforms.train else 3
    model = DenseNet(num_init_features=cfg.model.num_init_features, growth_rate=32, block_config=(6, 12, 32, 32),  #noqa
                     norm_type=cfg.model.norm_type, in_cnl=img_cnl, **kwargs)
    if cfg.model.load_pretrained_backbone and "Grayscale" not in cfg.dataset.datatransforms.train:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
...
def DenseNet201(cfg, **kwargs):
    ...
    """
    img_cnl = 1 if "Grayscale" in cfg.dataset.datatransforms.train else 3
    model = DenseNet(num_init_features=cfg.model.num_init_features, growth_rate=32, block_config=(6, 12, 48, 32),  # noqa
                     norm_type=cfg.model.norm_type, in_cnl=img_cnl, **kwargs)
    if cfg.model.load_pretrained_backbone and "Grayscale" not in cfg.dataset.datatransforms.train:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
...
def DenseNet161(cfg, **kwargs):
    ...
    """
    img_cnl = 1 if "Grayscale" in cfg.dataset.datatransforms.train else 3
    model = DenseNet(num_init_features=cfg.model.num_init_features, growth_rate=48, block_config=(6, 12, 36, 24),  #noqa
                     norm_type=cfg.model.norm_type, in_cnl=img_cnl, **kwargs)
    if cfg.model.load_pretrained_backbone and "Grayscale" not in cfg.dataset.datatransforms.train:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
