...
from model.backbone.DensenetModels import _get_norm as get_norm
...
class AttentionMap(nn.Module):
    ...
    def __init__(self, cfg, num_channels):
        ...
        self.pyramid_attention = FPAModule(num_channels, cfg.model.norm_type)

    def cuda(self, device=None):
        return self._apply(lambda t: t.cuda(device))

    def forward(self, feat_map):
        if self.cfg.model.attention_map == "CAM":
            return self.channel_attention(feat_map)
        elif self.cfg.model.attention_map == "SAM":
            return self.spatial_attention(feat_map)
        elif self.cfg.model.attention_map == "FPA":
            return self.pyramid_attention(feat_map)
        elif self.cfg.model.attention_map == "None":
            return feat_map
        else:
            Exception('Unknown attention type : {}'
                      .format(self.cfg.model.attention_map))
