...
class GlobalPool(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.lse_pool = LogSumExpPool(cfg.model.lse_gamma)

    def cuda(self, device=None):
        return self._apply(lambda t: t.cuda(device))

    def forward(self, feat_map, logit_map):
        if self.cfg.model.global_pool == 'AVG':
            return self.avgpool(feat_map)
        elif self.cfg.model.global_pool == 'MAX':
            return self.maxpool(feat_map)
        elif self.cfg.model.global_pool == 'PCAM':
            return self.pcampool(feat_map, logit_map)
        elif self.cfg.model.global_pool == 'AVG_MAX':
            a = self.avgpool(feat_map)
            b = self.maxpool(feat_map)
            return torch.cat((a, b), 1)
        elif self.cfg.model.global_pool == 'AVG_MAX_LSE':
            a = self.avgpool(feat_map)
            b = self.maxpool(feat_map)
            c = self.lse_pool(feat_map)
            return torch.cat((a, b, c), 1)
        elif self.cfg.model.global_pool == 'EXP':
            return self.exp_pool(feat_map)
        elif self.cfg.model.global_pool == 'LINEAR':
            return self.linear_pool(feat_map)
        elif self.cfg.model.global_pool == 'LSE':
            return self.lse_pool(feat_map)
        else:
            raise Exception('Unknown pooling type : {}'
                            .format(self.cfg.model.global_pool))
